...
void make_dshot_package()
{
    ...
    for ( int i = 19; i >= 0; i--)             // each digit in gcrnumber
    {
        gcr[7 + 20 - i + 1] = ((((gcrnumber &  1 << i )) >> i) ^ (gcr[7 + 20 - i] >> 6)) *
                              94; // exclusive ored with number before it multiplied by 64 to match output timer.
    }
    ...
    {
        gcr[7 + 20 - i + 1] = ((((gcrnumber &  1 << i )) >> i) ^ (gcr[7 + 20 - i] >> 6)) *
                              84; // exclusive ored with number before it multiplied by 64 to match output timer.
    }
